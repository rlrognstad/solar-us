...
uv run solar-fetch intraday 14      # last 14 days of 15-min production telemetry (default 7)
uv run solar-fetch consumption 14   # daily + last 14 days of 15-min consumption (needs CTs)
...
uv run solar-fetch balance          # self-consumption / self-sufficiency + import/export totals
uv run solar-fetch dashboard        # build dashboard.html (pass a path to override)
```

`weather` uses the free Open-Meteo API (no key, **does not** count against the Enphase
quota). `anomalies`, `balance`, and `dashboard` are offline — they read the cache and
make no network calls. Location comes from Enphase when available, else
`SOLAR_LAT`/`SOLAR_LON` in `.env`. `consumption` needs consumption CTs (check with
`solar-fetch meters`).
...
- `cache/intraday_production.parquet` — 15-min production telemetry
- `cache/daily_consumption.parquet` — daily consumption series
- `cache/intraday_consumption.parquet` — 15-min consumption telemetry
